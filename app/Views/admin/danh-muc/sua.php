<div class="space-y-6">
    <div>
        <h1 class="text-2xl font-bold text-foreground">Chỉnh sửa danh mục</h1>
        <p class="text-muted-foreground">
            Cập nhật thông tin danh mục: <?= htmlspecialchars($danhMuc->getTen()) ?>
        </p>
    </div>
                        ...
                        <div class="form grid gap-6">
                            <div class="grid gap-2">
                                ...
                                <input type="text" id="ten" name="ten" required
                                       ...
                                       value="<?= htmlspecialchars($_POST['ten'] ?? $danhMuc->getTen()) ?>">
                            </div>
                            ...
                            <div class="grid gap-2">
                                ...
                                <input type="text" id="duong_dan" name="duong_dan" required
                                       ...
                                       value="<?= htmlspecialchars($_POST['duong_dan'] ?? $danhMuc->getDuongDan()) ?>">
                            </div>

                            <div class="grid gap-2">
                                <label for="mo_ta">Mô tả</label>
                                <textarea id="mo_ta" name="mo_ta" rows="4"
                                          placeholder="Mô tả về danh mục"><?= htmlspecialchars($_POST['mo_ta'] ?? $danhMuc->getMoTa()) ?></textarea>
                            </div>
                            ...
                            <div class="grid gap-2">
                                ...
                                <input type="text" id="hinh_anh" name="hinh_anh"
                                       placeholder="https://..."
                                       value="<?= htmlspecialchars($_POST['hinh_anh'] ?? $danhMuc->getHinhAnh()) ?>">
                            </div>
                        </div>
                    </section>
                </div>
            ...
            <div class="space-y-6">
                                ...
                                <select id="danh_muc_cha_id" name="danh_muc_cha_id">
                                    ...
                                    <?php foreach ($parents as $parent): ?>
                                        <option value="<?= $parent->getId() ?>"
                                            <?= ((($_POST['danh_muc_cha_id'] ?? ($danhMuc->getDanhMucCha()?->getId() ?? '')) == $parent->getId())
                                                ? 'selected' : '') ?>>
                        ...
                        <div class="form grid gap-6">
                            <div class="grid gap-2">
                                ...
                                <input type="number" id="thu_tu" name="thu_tu" min="0" step="1"
                                       value="<?= htmlspecialchars($_POST['thu_tu'] ?? $danhMuc->getThuTu()) ?>">
                            </div>

                            <label class="flex items-center space-x-3">
                                <input type="checkbox" name="kich_hoat" value="1"
                                    <?= (isset($_POST['kich_hoat']) ? $_POST['kich_hoat'] : $danhMuc->isKichHoat())
                                        ? 'checked' : '' ?>>
                                <div>
                ...
                <div class="card">
                    <footer class="flex flex-col gap-3">
                        ...
                        <a href="/admin/danh-muc/chi-tiet/<?= $danhMuc->getId() ?>"
