<div class="max-w-4xl mx-auto p-6">
    ...
    <div class="text-center mb-8">
        <div class="mx-auto flex items-center justify-center w-20 h-20 bg-green-100 rounded-full mb-6">
            <svg class="w-10 h-10 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
            </svg>
        </div>
        <h1 class="text-3xl font-bold text-foreground mb-4">Đặt hàng thành công!</h1>
        ...
        <div class="space-y-4">
            <?php foreach ($donHang->getChiTiets() as $chiTiet): ?>
                <div class="flex items-center space-x-4 py-4 border-b border-border last:border-b-0">
                    <?php 
                    $hinhAnhChinh = $chiTiet->getBienTheSanPham()->getSanPham()->getHinhAnhChinh();
                    ...
                    <div class="flex-1">
                        <h4 class="font-medium text-foreground"><?= htmlspecialchars($chiTiet->getTenSanPham()) ?></h4>
                        <p class="text-sm text-muted-foreground">
                            <?php 
                            $chiTietBienThe = $chiTiet->getChiTietBienThe();
                            if ($chiTietBienThe) {
                                $details = [];
                                if (!empty($chiTietBienThe['bo_nho'])) {
                                    $details[] = $chiTietBienThe['bo_nho'];
                                }
                                if (!empty($chiTietBienThe['mau_sac'])) {
                                    $details[] = $chiTietBienThe['mau_sac'];
                                }
                                echo htmlspecialchars(implode(' - ', $details));
                            }
                            ?>
                        </p>
                        <p class="text-sm text-muted-foreground">Số lượng: <?= $chiTiet->getSoLuong() ?></p>
                    </div>
                    
                    <div class="text-right">
                        <p class="font-medium text-foreground"><?= number_format($chiTiet->getGiaDonVi(), 0, ',', '.') ?> ₫</p>
                        <p class="text-sm text-muted-foreground">Tổng: <?= $chiTiet->getTongGiaFormatted() ?></p>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    <div class="text-center space-y-4">
        ...
        <?php 
        $thanhToan = $donHang->getThanhToans()->first();
        $paymentMethod = $thanhToan ? $thanhToan->getCongThanhToan() : '';
        ?>
            ...
            <div class="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6">
                <div class="flex items-center justify-center space-x-2 text-orange-800">
                    ...
                    <p class="text-sm font-medium">
                        Bạn sẽ thanh toán bằng tiền mặt khi nhận hàng. Vui lòng chuẩn bị đủ tiền mặt: <span class="font-bold"><?= $donHang->getTongTienFormatted() ?></span>
                    </p>
                </div>
            </div>
        <?php elseif (in_array($paymentMethod, ['momo_wallet', 'zalopay_wallet'])): ?>
            <div class="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-6">
                <div class="flex items-center justify-center space-x-2 text-purple-800">
                    ...
                    <p class="text-sm font-medium">
                        Đơn hàng đang chờ thanh toán qua ví điện tử. Chúng tôi sẽ liên hệ với bạn để hướng dẫn thanh toán.
                    </p>
                </div>
            </div>
        <?php elseif ($paymentMethod === 'bank_transfer'): ?>
            <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                <div class="flex items-center justify-center space-x-2 text-green-800">
                    ...
                    <p class="text-sm font-medium">
                        Vui lòng chuyển khoản theo thông tin chúng tôi sẽ gửi qua email. Đơn hàng sẽ được xử lý sau khi nhận được thanh toán.
                    </p>
                </div>
            </div>
        <?php endif; ?>
